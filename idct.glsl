...
layout(std140, binding = 2) readonly buffer qtable_buffer {
	float qtable[];
};

uniform layout(binding = 3, r8) writeonly image2d out_plane;
...
void blit_subblock(float dct[64], uint dx, uint dy) {
	for (int row = 0; row < 8; row++) {
		int src_offset = row * 8;

		for (int column = 0; column < 8; column++) {
			float f = dct[src_offset + column] + 128.0;
			imageStore(out_plane, uint2(dx + column, dy + row), clamp(f, 0.0, 255.0));
		}
	}
}
...
void main() {
	...
	blit_subblock(dct0, bx, by);
	blit_subblock(dct1, bx + 8, by);
	blit_subblock(dct2, bx, by + 8);
	blit_subblock(dct3, bx + 8, by + 8);
}